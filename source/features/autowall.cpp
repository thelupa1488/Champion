...
std::optional<CAutoWall::wall_pen> CAutoWall::FireBullet(C_BaseCombatWeapon* pWeapon, Vector& direction, float& currentDamage)
{
	...
	while (possibleHitsRemaining > 0 && currentDamage >= 1.f)
	{
		...
		bool isEnemy = g_Options.aimbot[g_LocalPlayer->m_hActiveWeapon()->m_iItemDefinitionIndex()].deathmatch ? true : (g_LocalPlayer->m_iTeamNum() != static_cast<C_BasePlayer*>(enterTrace.hit_entity)->m_iTeamNum());
