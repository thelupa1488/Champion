...
{
	static auto key_values = reinterpret_cast<void(__thiscall*)(void*, const char*)>(Utils::PatternScan(GetModuleHandleW(L"client_panorama.dll"), "55 8B EC 51 33 C0 C7 45 ? ? ? ? ? 56 8B F1 81 26 ? ? ? ? C6 46 03 ? 89 46 10 89 46 18 89 46 14 89 46 1C 89 46 04 89 46 08 89 46 0C FF 15 ? ? ? ? 6A 01 FF 75 08 8D 4D FC 8B 10 51 8B C8 FF 52 24 8B 0E 33 4D FC 81 E1 ? ? ? ? 31 0E 88 46 03"));
	key_values(this, name);
}
void KeyValues::SetUint64(const char* keyName, int value, int value2)
{
	static auto key_values_set_uint64 = reinterpret_cast<void(__thiscall*)(void*, const char*, int, int)>(Utils::PatternScan(GetModuleHandleW(L"client_panorama.dll"), "55 8B EC 56 6A 01 FF 75 08"));
...
{
	static auto key_values_get_int = reinterpret_cast<int(__thiscall*)(void*, const char*, int)>(Utils::PatternScan(GetModuleHandleW(L"client_panorama.dll"), "55 8B EC 6A ? FF 75 08 E8 ? ? ? ? 85 C0 74 45"));
